#!/usr/bin/env python3
"""
Introduction 템플릿을 Notion 페이지에 업로드

사용법:
    python upload_template_to_notion.py <page_id>
    
예시:
    python upload_template_to_notion.py 2a841454561d8084a7c3cf21eefc6e07
    또는
    python upload_template_to_notion.py https://www.notion.so/2a841454561d8084a7c3cf21eefc6e07
"""
...
def parse_markdown_to_blocks(md_content: str, max_blocks: int = 100) -> List[List[Dict]]:
    ...
    while i < len(lines):
        ...
        # Checkbox (- [ ] or - [x])
        elif re.match(r'^[-*]\s+\[([ xX])\]\s+', line.strip()):
...
def main():
    print("="*60)
    print("Notion 업로드: Introduction 템플릿")
    print("="*60)
    ...
    md_file = os.path.join(script_dir, 'notion_introduction_template.md')
